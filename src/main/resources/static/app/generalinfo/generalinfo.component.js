"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var generalinfo_1 = require('./generalinfo');
var backend_service_1 = require('../backend/backend.service');
var GeneralinfoComponent = (function () {
    function GeneralinfoComponent(backendService) {
        this.backendService = backendService;
        this.generalinfo = new generalinfo_1.Generalinfo();
    }
    GeneralinfoComponent.prototype.getInfo = function () {
        var _this = this;
        this.backendService
            .getGeneralinfo()
            .then(function (generalinfo) {
            _this.generalinfo = generalinfo;
        });
    };
    GeneralinfoComponent.prototype.ngOnInit = function () {
        this.getInfo();
    };
    GeneralinfoComponent.prototype.handleError = function (error) {
        console.error('An error occurred', error); // for demo purposes only
        return Promise.reject(error.message || error);
    };
    GeneralinfoComponent = __decorate([
        core_1.Component({
            selector: 'generalinfo',
            templateUrl: 'app/generalinfo/generalinfo.html'
        }), 
        __metadata('design:paramtypes', [backend_service_1.BackendService])
    ], GeneralinfoComponent);
    return GeneralinfoComponent;
}());
exports.GeneralinfoComponent = GeneralinfoComponent;
//# sourceMappingURL=generalinfo.component.js.map